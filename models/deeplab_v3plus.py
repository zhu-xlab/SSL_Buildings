...
from utils import upsample_scale, upsample_size
...
class Deeplab_V3plus(nn.Module):
    def __init__(self, backbone='resnet50', num_classes=6):
        ...
        self.decoder_aux = Decoder(num_classes, nn.BatchNorm2d)
        self.scale_factor = 4

    def forward(self, x):
        feats = self.get_feats(x)
        probs = self.get_probs(feats)
        return probs

    def get_feats(self, x):
        feat, low_level_feat = self.backbone(x)
        feat = self.aspp(feat)
        return [feat, low_level_feat]       

    def get_probs(self, feats):
        [feat, low_level_feat] = feats
        pred = self.decoder(feat, low_level_feat)
        pred = upsample_scale(pred, self.scale_factor)
    ...
    def get_10x_lr_params(self):
        ...
        b.append(self.decoder_aux.parameters())
        for j in range(len(b)):
            for i in b[j]:
                yield i

    def optim_parameters(self, init_lr):
        return [{'params': self.get_1x_lr_params(), 'lr': init_lr},
                {'params': self.get_10x_lr_params(), 'lr': 10 * init_lr}]

    def adjust_learning_rate(self, init_lr, optimizer, curr_iter, total_iters):
        lr = init_lr * (  (1-float(curr_iter)/total_iters) ** 0.9  )
