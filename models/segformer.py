...
class SegFormerHead(nn.Module):
    ...
    def __init__(self, num_classes=20, in_channels=[32, 64, 160, 256], embedding_dim=768, dropout_ratio=0.1):
        ...
        self.linear_pred_aux = self._make_last_layer(embedding_dim*4, num_classes)
...
class SegFormer(nn.Module):
    def __init__(self, num_classes=2, phi='b2', pretrained=True):
    ...
    def get_feats(self, input):
        feat = self.backbone.forward(input)
        feat = self.decoder.fuse_features(feat)
        return feat

    def get_probs(self, feat):
        pred = self.decoder.get_predicts(feat)
    ...
    def optim_parameters(self, init_lr):
        return [{'params': self.get_1x_lr_params(), 'lr': init_lr},
                {'params': self.get_10x_lr_params(), 'lr': init_lr * 10}]
